...
public sealed class MainWindow : Window
{
    ...
    private Control BuildAccount()
    {
        ...
        static string StateText(AccountState state) => state switch { SigningInAccount => "Signing in… Current stage: Microsoft", MicrosoftAuthenticatedAccount => "Microsoft authenticated. Current stage: Xbox Live", XboxAuthenticatedAccount => "Xbox Live authenticated. Current stage: XSTS", XstsAuthenticatedAccount => "XSTS authenticated. Current stage: Minecraft", MinecraftAuthenticatedAccount => "Minecraft authenticated. Current stage: Entitlement", OwnershipConfirmedAccount => "Minecraft: Java Edition ownership confirmed. Current stage: Profile", ProfileLoadedAccount => "Minecraft profile loaded. Finalizing sign-in…", SignedInAccount signed => $"Signed in and ready. Session expires {signed.Expiry.ExpiresAt:u}.", AuthenticationFailed failed => $"Sign-in failed ({failed.Code}): {failed.SafeMessage}", SigningOutAccount => "Signing out…", _ => "Signed out" };
        var status = new TextBlock { Text = StateText(authentication.CurrentState), TextWrapping = TextWrapping.Wrap }; var details = new SelectableTextBlock { TextWrapping = TextWrapping.Wrap };
        ...
        _ = Task.Run(async () => { await foreach (var state in authentication.ObserveAccountStateAsync()) Dispatcher.UIThread.Post(() => status.Text = StateText(state)); });
    ...
    private async Task BuildDiagnosticsAsync()
    {
        ...
        var export = new Button { Content = "Export redacted authentication diagnostics" }; var exportStatus = new TextBlock(); export.Click += async (_, _) => { var report = AuthenticationDiagnosticsExporter.Create(authenticationConfiguration, OperatingSystem.IsWindows(), authentication.CurrentState.GetType().Name, authentication.CurrentState is AuthenticationFailed failure ? failure.Code : "None", verified.Verified, scenarios); await diagnosticsExporter.ExportAsync(Path.Combine(paths.Data, "authentication-diagnostics-redacted.json"), report); exportStatus.Text = "Redacted report exported as authentication-diagnostics-redacted.json in launcher application data."; }; rows.Children.Add(export); rows.Children.Add(exportStatus);
